...
ssh $REMOTE_USER@$REMOTE_HOST "mysqldump --no-data --defaults-file=.creds-remote.cfg --skip-lock-tables -h $REMOTE_DB_HOST -P $REMOTE_DB_PORT --protocol=tcp <schema> | sed 's/^CREATE TABLE /CREATE TABLE IF NOT EXISTS /' > <schema> $IGNORED_TABLES_STRING $REMOTE_DB_SCHEMA" > $DUMP_FILE
